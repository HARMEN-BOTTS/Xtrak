...
class Admin extends Component
{
    ...
    {
        $this->datas = TrgEvent::latest()->get();
    }
    ...
    public function deleteSelected()
    {
        ...
        TrgEvent::whereIn('id', $this->selectedRows)->delete();
        $this->selectedRows = [];
    ...
    {
        $event = TrgEvent::with('trgDashboard')->find($eventId);
        ...
        $this->eventFormData = [
            'trg_id' => $event->trg_id,
            'trg_code' => $event->trgDashboard->trg_code ?? '',
            'company' => $event->trgDashboard->company ?? '',
            'ctc_code' => $event->trgDashboard->ctc_code ?? '',
            'first_name' => $event->trgDashboard->first_name ?? '',
            'last_name' => $event->trgDashboard->last_name ?? '',
            'position' => $event->trgDashboard->position ?? '',
    ...
    public function resetEventForm()
    {
        if ($this->isEditMode) {
            ...
            $selectedItem = null;
            if (!empty($this->selectedRows)) {
                $selectedItem = \App\Models\Trgdashboard::find($this->selectedRows[0]);
            }
